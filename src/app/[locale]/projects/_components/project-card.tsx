    ...
    <Card ref={ref}>
      <CardContent className="md:flex h-full">
        <div className="space-y-4 pt-6 flex flex-col justify-between">
          <div className="relative w-full h-48 md:h-60 lg:h-72 xl:h-80">
            <Image src={Array.isArray(props.image) ? props.image[0] ?? "" : props.image ?? ""} alt={props.title} fill priority className="object-cover rounded-md" />
          </div>
          ...
          <div className="flex space-x-4">
            ...
            <Dialog>
              ...
              <DialogContent className="w-full h-full p-4 flex justify-center items-center">
                ...
                <div className={`relative w-[1200px] h-[250px] max-h-[400px] md:w-[500px] md:min-h-[290px] lg:w-[1200px] lg:min-h-[700px] overflow-y-auto ${props.isMobile ? "pt-20 md:rotate-0 w-[300px] h-full" : "rotate-90"} md:rotate-0 flex justify-center`}>
                  {
                    Array.isArray(props.image) ? (
                      props.image.map((img, index) => (
                        <Image key={index} src={img} alt={props.title} fill priority className="object-cover" />
                      ))
                    ) : (
                      <Image src={props.image ?? ""} alt={props.title} fill priority className="object-cover" />
                    )
                  }
                </div>
              </DialogContent>
